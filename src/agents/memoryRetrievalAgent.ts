 ...
 */
export function createMemoryRetrievalAgent(
  memoryPath: string = '.sea/engineering_memory.md'
 ...
 */
function parseMemoryLog(content: string): MemoryEntry[] {
...
function parseMemoryEntry(block: string): MemoryEntry | null {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith('### Request')) {
      ...
      switch (currentSection) {
        ...
        case 'decision':
          // Single line fields
          if (currentSection === 'request') entry.request = item;
          else entry.decision = item;
          break;
        case 'whatWorked':
...
function formatMemoryContext(entries: MemoryEntry[], request: string): string {
  const lines: string[] = [
