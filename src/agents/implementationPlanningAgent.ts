...
export function createImplementationPlanningAgent(): (
  ...
  return async (state: WorkspaceState) => {
    ...
    const componentPlans = createComponentPlans(
      impactAnalysis,
      requirement,
      architecturePlan,
      workspace
...
function createExecutionGroups(
  ...
  const groups: ExecutionGroupPlan[] = [
    {
      groupId: 'modify',
...
function createComponentPlans(
  impactAnalysis: WorkspaceState['impactAnalysis'],
  requirement: WorkspaceState['requirement'],
  architecturePlan: WorkspaceState['architecturePlan'],
  workspace: WorkspaceState['workspace']
): ComponentImplementationPlan[] {
  if (!impactAnalysis) return [];

  const plans: ComponentImplementationPlan[] = [];
  ...
  for (const component of impactAnalysis.affectedComponents) {
    ...
    const componentConfig = workspace.components?.find(
...
function createComponentPlan(
  component: { component: string; changeRole: string },
  requirement: WorkspaceState['requirement'],
  architecturePlan: WorkspaceState['architecturePlan'],
  componentConfig?: WorkspaceState['workspace']['components'][0]
): ComponentImplementationPlan {
  ...
  return {
    ...
    changeRole: component.changeRole as ComponentImplementationPlan['changeRole'],
    steps,
    allowedPaths,
    protectedPaths,
    forbiddenPaths,
...
function generateDefinitionOfDone(
  changeRole: string,
  requirement?: WorkspaceState['requirement']
): string[] {
  const dod: string[] = [
...
function generateVerificationExpectations(
  changeRole: string,
