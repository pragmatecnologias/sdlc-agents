...
async function hasBuildFile(componentPath: string, kind: string): Promise<boolean> {
  const buildFilePatterns: Record<string, string[]> = {
...
async function findBuildFiles(workspacePath: string): Promise<string[]> {
  const patterns = [
    'package.json',
    'pom.xml',
    'build.gradle',
...
async function findTestFolders(workspacePath: string): Promise<string[]> {
  const testPatterns = [
    '**/test',
    '**/tests',
    '**/__tests__',
...
function detectFrameworks(buildFiles: string[]): string[] {
  const frameworks: string[] = [];

  const frameworkIndicators: Record<string, RegExp[]> = {
