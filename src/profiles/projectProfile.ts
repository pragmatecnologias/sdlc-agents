...
export interface ProfileDefinition {
  ...
  description: string;
  detect: (context: DetectionContext) => number; // confidence 0-1
 ...
 */
export const PROFILE_DEFINITIONS: Record<ProjectProfileName, ProfileDefinition> = {
  ...
  SPRING_BOOT_SERVICE: {
    ...
    detect: (ctx) => {
      ...
      const hasSpring = ctx.buildFiles.some(f =>
        f.includes('application.properties') || f.includes('application.yml') ||
  ...
  LIBRARY_PACKAGE: {
    ...
    detect: (ctx) => {
      const hasPackageJson = ctx.buildFiles.some(f => f.endsWith('package.json'));
      const hasPackageBuild = ctx.buildFiles.some(f =>
        f.includes('tsconfig') || f.includes('webpack') || f.includes('rollup')
      );
  ...
  MICROSERVICES_WORKSPACE: {
    ...
    detect: (ctx) => {
      const hasMultipleServices = ctx.componentKinds.filter(k =>
        k === 'backend' || k === 'service'
      ).length >= 2;
  ...
  INFRASTRUCTURE_REPO: {
    ...
    detect: (ctx) => {
      const hasIaC = ctx.buildFiles.some(f =>
        f.includes('terraform') || f.includes('.tf') ||
 ...
 */
export function getProfileNames(): ProjectProfileName[] {
  return Object.keys(PROFILE_DEFINITIONS) as ProjectProfileName[];
}

/**
 * Get profile definition
 */
export function getProfileDefinition(name: ProjectProfileName): ProfileDefinition | undefined {
