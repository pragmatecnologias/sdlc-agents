 ...
 */
import { getGitDiff, getGitStatus } from '../tools/gitTool.js';
...
export function createDiffInspectorAgent() {
    return async (state) => {
        ...
        for (const [componentName, componentState] of Object.entries(componentStates || {})) {
            ...
            if (!component)
                continue;
            const result = await inspectComponentDiff(state.runId, componentName, component.path, component.protectedPaths || [], component.forbiddenPaths || [], '.sea');
...
async function inspectComponentDiff(runId, componentName, componentPath, protectedPaths, forbiddenPaths, baseDir) {
...
function checkForbiddenViolations(changedFiles, forbiddenPaths) {
...
function checkProtectedViolations(changedFiles, protectedPaths) {
...
//# sourceMappingURL=diffInspectorAgent.js.map