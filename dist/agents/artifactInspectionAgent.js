...
export function createArtifactInspectionAgent() {
    return async (state) => {
        ...
        for (const [componentName, componentState] of Object.entries(componentStates || {})) {
            ...
            if (!component || !component.artifact)
                continue;
            // Only inspect if component was modified
            if (componentState.changeRole === 'no_change')
                continue;
            try {
                const report = await inspectArtifact(component.path, component.artifact.type);
...
//# sourceMappingURL=artifactInspectionAgent.js.map