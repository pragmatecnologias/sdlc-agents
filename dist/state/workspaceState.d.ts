...
export type RunStatus = 'initialized' | 'running' | 'awaiting_approval' | 'paused' | 'completed' | 'failed' | 'aborted';
/**
...
export type ChangeRole = 'modify' | 'verify_only' | 'package_only' | 'artifact_verify' | 'no_change' | 'blocked' | 'unknown';
/**
 * Architecture plan
 */
...
export type ArchitectureDecision = 'proceed' | 'proceed_with_constraints' | 'revise_request' | 'reject' | 'blocked';
 ...
 */
export type ProjectProfileName = 'SINGLE_REPO_FRONTEND' | 'SINGLE_REPO_BACKEND' | 'MONOREPO_WEB_APP' | 'MULTI_REPO_ENTERPRISE_APP' | 'WAR_COMPOSITE_APP' | 'SPRING_BOOT_SERVICE' | 'NODE_API' | 'REACT_APP' | 'ANGULAR_APP' | 'VUE_APP' | 'CHROME_EXTENSION' | 'THREEJS_GAME' | 'PYTHON_CLI' | 'LIBRARY_PACKAGE' | 'MICROSERVICES_WORKSPACE' | 'INFRASTRUCTURE_REPO' | 'DOCUMENTATION_REPO' | 'CUSTOM';
/**
 * Component kind types
 */
export type ComponentKind = 'ui' | 'frontend' | 'backend' | 'service' | 'assembly' | 'package' | 'library' | 'infra' | 'test' | 'docs' | 'game' | 'cli' | 'contract' | 'generated' | 'unknown';
/**
 * Component role types
 */
export type ComponentRole = 'source' | 'application' | 'service' | 'packager' | 'assembler' | 'contract' | 'test-suite' | 'infrastructure' | 'documentation' | 'generated' | 'verification-only';
/**
 * Create initial workspace state for a new run
 */
export declare function createInitialWorkspaceState(runId: string, userRequest: string, workspace: WorkspaceConfig): WorkspaceState;
//# sourceMappingURL=workspaceState.d.ts.map